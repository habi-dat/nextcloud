...
then
	...
	((COUNT+=1))
	EXTERNAL_SITES+=$(printf "$EXTERNAL_SITES_TEMPLATE" "$COUNT" "wiki.png" "$COUNT" "Mediawiki" "$HABIDAT_PROTOCOL://$HABIDAT_MEDIAWIKI_SUBDOMAIN.$HABIDAT_DOMAIN")
fi

if [ ! -z "$HABIDAT_DOKUWIKI_SUBDOMAIN" ]
then
	if [ "$COUNT" -ne 0 ]
	then
		EXTERNAL_SITES+=","
	...
	((COUNT+=1))
	EXTERNAL_SITES+=$(printf "$EXTERNAL_SITES_TEMPLATE" "$COUNT" "wiki.png" "$COUNT" "Dokuwiki" "$HABIDAT_PROTOCOL://$HABIDAT_DOKUWIKI_SUBDOMAIN.$HABIDAT_DOMAIN")
fi
